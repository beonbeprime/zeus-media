...
PAD_SIDES = 1.3       # largura do crop = largura do rosto * PAD_SIDES (margem lateral)
...
def eye_centered_crop(face, frame_w, frame_h, slot_w, slot_h,
                      eye_frac=EYE_FRAC, pad_sides=PAD_SIDES):
    """
    ...
    HORIZONTAL:
      ...
      - Largura = face_width * PAD_SIDES para dar margem lateral
    ...
    # Largura do crop: face + margem, mas nunca menor que o slot
    crop_w = max(int(fw * pad_sides), slot_w)
...
def build_filter_complex(segments, crop_host, crop_guest, out_w, out_h):
    ...
    for i, (start, end) in enumerate(segments):
        ...
        # Host (cima): crop -> scale proporcional -> crop para slot exato
        lines.append(
            f"[v{i}a]crop={cw1}:{ch1}:{cx1}:{cy1},"
            f"scale={out_w}:-2:flags=lanczos,"
            f"crop={out_w}:{slot_h}:0:(ih-{slot_h})/2[top{i}];"
        )
        ...
        lines.append(
            ...
            f"scale={out_w}:-2:flags=lanczos,"
            f"crop={out_w}:{slot_h}:0:(ih-{slot_h})/2[bot{i}];"
        )
    ...
    # Concat final
    n = len(segments)
    lines.append(f"{''.join(outs)}concat=n={n}:v=1:a=0[vout]")
...
def reframe(video_path, output_path, segments=None, draft=True, flip_sides=False):
    ...
    print(f"  Host   (cima):     crop={cw1}x{ch1} em x={cx1},y={cy1}"
          .replace("cw1", str(crop_host[2])).replace("ch1", str(crop_host[3]))
          .replace("cx1", str(crop_host[0])).replace("cy1", str(crop_host[1])))
    print(f"  Guest  (baixo):    crop={cw2}x{ch2} em x={cx2},y={cy2}"
          .replace("cw2", str(crop_guest[2])).replace("ch2", str(crop_guest[3]))
          .replace("cx2", str(crop_guest[0])).replace("cy2", str(crop_guest[1])))
